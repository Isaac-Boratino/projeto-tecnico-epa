...
Além da notificação remota via Telegram, o sistema conta com um buzzer conectado a uma saída digital do ESP32, que emite um som quando uma falha é detectada em qualquer um dos postes. Esse retorno imediato e local é útil especialmente para a demonstração na Feira EPA, permitindo que quem estiver observando a maquete perceba instantaneamente, mesmo sem olhar para um celular ou painel, o momento exato em que o sistema identifica o problema.

## Integração entre os Componentes

Na prática, sensores e atuadores trabalham em conjunto formando o ciclo de simulação e detecção do sistema: o botão (entrada) interfere fisicamente no LED (atuador), o LDR (sensor) capta essa mudança de luminosidade de forma independente, e o ESP32 processa essa leitura para acionar os atuadores de resposta — o próprio LED (mantendo o estado real do poste), o buzzer (alerta sonoro imediato) e, por fim, o envio da notificação para a camada de conectividade. Esse desenho garante que a maquete não seja uma simulação artificial de alarme, mas replique o comportamento real de um sistema de monitoramento por sensoriamento de luz.
